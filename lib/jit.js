...
(function(){
  ...
  GLOBALENV = gensymbol({pre:"global_context_"});
  ...
  function parseVariables(s,ctx){
    return s.replace(regvar,function(m,a){
             if(ctx[m]){
               return GLOBALENV+'.'+m;
             }else{
               return ENV+'.'+m;
  ...
  function _compile(list,code,indent,ctx){
    ...
    if(list[0] === "if"){
      ...
      for(var i1=0,l1=ifexps.length;i1<l1;i1++){
        ...
        var predict = ifexp[0];
        var parsedVars = parseVariables(predict[0],ctx);
        code = code + parsedVars;
        code = code + ")";
        code = code + "{\n";
        var ifbody = ifexp[1];
      ...
      if(ctx[list[1]]){//判断是否为局部变量
        ret = indent + '__s__ += ' + GLOBALENV + "." + list[1];
      }else{
  ...
  //将一段模板语法，编译为js代码
  function compile(lists,ctx){
    if(ctx == undefined){
      ctx = this;
    }
    var js = to_js(lists,ctx);
    var global_context_name = GLOBALENV;
    global_context_name = ctx;
    return eval('('+js+')');
  }

  function to_js(lists,ctx){
